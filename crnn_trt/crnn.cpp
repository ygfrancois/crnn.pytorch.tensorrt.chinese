...
static const int NUM_CLASS = 6862;
static const int OUTPUT_W = INPUT_W/4 +1;
static const int OUTPUT_SIZE = OUTPUT_W * NUM_CLASS;  // 41=160/4+1, 15=class_num, find all 41(w) and 15(class_num) in this file and change it to new w and class_num
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    sfl0->setReshapeDimensions(Dims4{OUTPUT_W, 1, 1, 512});
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::cerr << "./crnn -d ../../images  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    cv::Mat img = cv::imread(img_dir + "/" + "demo.png");
    if (img.empty()) {
    ...
    for (int i = 0; i < OUTPUT_W; i++) {
        ...
        for (int j = 1; j < NUM_CLASS; j++) {
            if (prob[NUM_CLASS * i + j] > prob[NUM_CLASS * i + maxj]) maxj = j;
        }
